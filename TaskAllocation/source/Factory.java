...
public class Factory {
	...
	public static Task createTask() {
		...
		NewTask.value = NewTask.Task_Resources.getValue()
				* (100 + Functions.getRandom(Task.Valuedown, Task.Valueup))
				/ 100.00;
		NewTask.extra = Task.MinExtra + (Task.MaxExtra - Task.MinExtra)
				* Functions.getRandom(0, 100) / 100;
		NewTask.Deadline = Functions.getRandom(Task.MinDeadline,
				Task.MaxDeadline);
		NewTask.AllocatedAgents.add(Experiment.experiment.getAgents().get(
				Functions.getRandom(0, Allocation.Number_Agent - 1)));
		return NewTask;
	}
	...
	private static Resource createTask_Resources(Task newTask) {
		...
		newTask.TotalNumber_Resource = Functions.getRandom(Task.MinResource,
				Task.MaxResource);
		taskresource.Number_Resource = new int[Resource.Number_Types];
		for (int i = 0; i < newTask.TotalNumber_Resource; i++) {
			int randomnumber = Functions
					.getRandom(0, Resource.Number_Types - 1);
	...
	private static Resource createAgent_Resources(Agent newAgent) {
		...
		newAgent.TotalNumber_Resource = Functions.getRandom(Agent.MinResource,
				Agent.MaxResource);
